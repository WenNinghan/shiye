...
try {
    ...
    if ($WithFormula) {
        ...
        & $CorePython packaging/build_model_pack.py --runtime (Join-Path $buildRoot 'formula/shiye-formula') --cache $FormulaCache --output (Join-Path $releaseRoot 'Shiye-Formula-0.3.0-Windows-x64.shiye-model') --manifest desktop/trusted-models.json
    ...
    try {
        ...
        & npm.cmd run dist -- "--config.directories.output=$releaseRoot"; Assert-Step 'Desktop installer'
    } finally { Pop-Location }
