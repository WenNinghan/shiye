...
## 后续进展：无 IPP 候选已完成本机整合

上节为当时的停止点，现由 [0.3.1 候选验收](offline-candidate-0.3.1.md) 更新：两套云端构建及本机冻结完成，新的本地安装候选/模型包已经生成，独立可信清单和模型导入通过；根 MIT 与旧清单未改。新的 OpenCV 扩展身份、禁用参数及回归均有证据，原 OpenCV IPP 风险不能再直接套用到这个新核心候选。

新材料包括 Paddle 固定 MKLML 归档的原始声明，与实际 DLL 身份匹配；不将其自动归为 Apache。其余原生条款、对应源码及缺失许可正文仍需完成覆盖核对，未宣布全包分发通过。新产物仅保存在本机 G 盘，未公开 Release。
